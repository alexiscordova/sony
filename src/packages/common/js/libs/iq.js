  ...
  var LOCAL_STORAGE_KEY = 'iqjs',
    ...
    getBreakpoint = function(breakpoints, vWidth) {
      ...
      while (_breakpoint = breakpoints[i]) {
        ...
        if (vWidth > 0) {
          if (
          minWidth && maxWidth && vWidth >= minWidth && vWidth <= maxWidth || minWidth && !maxWidth && vWidth >= minWidth || maxWidth && !minWidth && vWidth <= maxWidth) {
    ...
    isInViewportRange = function(img, asyncDistance) {
      ...
      var inView = (ct_top - asyncDistance) <= (img_offset.top + img_height) && (ct_bottom + asyncDistance) >= img_offset.top && (ct_left - asyncDistance) <= (img_offset.left + img_width) && (ct_right + asyncDistance) >= img_offset.left;
