...
(function($, Modernizr, window, undefined) {
    ...
    var SonyCarousel = function (element, options) {
        ...
        // init modules
        $.each($.scModules, function (helper, opts) {
            opts.call(self);
        });
        ...
        self.slider.addClass( (self._slidesHorizontal ? 'scHor' : 'scVer') + (self._isMove ? '' : ' scFade') );

        var sliderHTML = '<div class="scOverflow"><div class="scContainer">';
        ...
        var slides = self._slidesContainer.children('.scSlide');
        ...
        if('ontouchstart' in window || 'createTouch' in document) {
            self.hasTouch = true;
            self._downEvent = 'touchstart.sc';
            self._moveEvent = 'touchmove.sc';
            self._upEvent = 'touchend.sc';
            self._cancelEvent = 'touchcancel.sc';
            ...
            self._downEvent = 'mousedown.sc';
            self._moveEvent = 'mousemove.sc';
            self._upEvent = 'mouseup.sc';
            self._cancelEvent = 'mouseup.sc';
        }
        ...
        if(self._useCSS3Transitions) {
            ...
            if(self._use3dTransform) {
                if(isWebkit) {
                    self.slider.addClass('scWebkit3d');
                }
        ...
        self.ev.trigger('scAfterPropsSetup'); // navigation (bullets, thumbs...) is created here
        ...
        if(self.st.arrowsNav) {
            ...
            if(self.st.arrowsNavAutoHide && !self.hasTouch) {
                self._arrowLeft.addClass('scHidden');
                self._arrowRight.addClass('scHidden');

                var hoverEl = container;
                hoverEl.one("mousemove.arrowshover",function() {
                    self._arrowLeft.removeClass('scHidden');
                    self._arrowRight.removeClass('scHidden');           
                });
        ...
        var videoClasses = ['scPlayBtnIcon', 'scPlayBtn', 'scCloseVideoBtn', 'scCloseVideoIcn'];
            self._slidesContainer.click(function(e) {
                if(!self.dragSuccess) {
                    ...
                    if(self.st.navigateByClick && !self._blockActions) {
                        if($(e.target).closest('.scNoDrag', self._currHolder).length) {
                             return true;
                        }
                        self._mouseNext(e);
                    } 
        ...
        self.ev.trigger('scAfterInit');
    } /* SonyCarousel Constructor End */

    /**
     *
     * SonyCarousel Core Prototype
     * 
     */
    SonyCarousel.prototype = {
        ...
        _parseNode: function(content, pushToSlides) {
            ...
            self.ev.trigger('scBeforeParseNode', [content, obj]);
            if(obj.stopParsing) {
                return;
            }
            ...
            if(!obj.hasCover) {
                if(content.hasClass('scImg')) {
                    tempEl = content;
                    hasImg = true;
                } else {
                    tempEl = content.find('.scImg');
                    if(tempEl.length) {
                        hasImg = true;
                    }
                }
                ...
                if(hasImg) {
                    obj.bigImage = tempEl.attr('data-scBigImg');
                    if(tempEl.is('a')) {
                        parseEl(tempEl, 'href');
                    ...
                    content.addClass('scImg');
                    parseEl(content);
                }
            }
            tempEl = content.find('.scCaption');
            if(tempEl.length) {
            ...
            obj.content = content;
            self.ev.trigger('scAfterParseNode', [content, obj]);
            function parseEl(el, s) {
                obj.image = el.attr(!s ? 'src' : s);
                obj.caption = !s ? el.attr('alt') : el.contents();
                obj.videoURL = el.attr('data-scVideo');
            }
            if(pushToSlides) {
        ...
        _bindKeyboardNav: function() {
            var self = this;
            self._doc.on('keydown.sckb', function(e) {
        ...
        destroy: function(remove) {
            var self = this;
            self.ev.trigger('scBeforeDestroy');
            self._doc.off('keydown.sckb ' + self._moveEvent +' '+ self._upEvent );
            self._slidesContainer.on(self._downEvent, function(e) { self._onDragStart(e); });   
            self.slider.data('sonyCarousel', '');
            if(remove) {
                self.slider.remove();
            }
        },
        ...
        setItemHtml: function(currSlideObject, beforeTransition) {
                ...
                return function (e) {
                    ...
                    if(e) {
                        ...
                        if(e.type === 'error') {
                            ...
                            content.addClass('scSlideError');
                            holder.html(content);
                            slideObject.holder.trigger('scAfterContentSet');
                            self.ev.trigger('scAfterContentSet', slideObject);
                            return;
                        }
                    }
                    ...
                    if(slideObject.image) {
                        ...
                        if(!slideObject.isLoaded) {
                            ...
                            if(tempEl.length && tempEl.is('a')) {
                                ...
                                if(isRoot) {
                                    content = $('<img class="scImg" src="'+slideObject.image+'" />');
                                } else {
                                    content.find('.scImg').replaceWith('<img class="scImg" src="'+slideObject.image+'" />');
                                }
                    ...
                    holder.find('a').off('click.sc').on('click.sc', function(e) {   
                        if(self.dragSuccess) {                      
                            return false;
                        } else {
                            self._blockActions = true;
                            self.ev.trigger('scSlideClick');
                            //e.stopPropagation();
            ...
            function parseDataAndLoad() {
                ...
                if(!currSlideObject.isLoading) {
                    ...
                    var guard = 1;
                    var loopUntilLoaded = function (sizeType) {
        ...
        _addBlockToContainer: function(slideObject, content, dontFade) {
            ...
            if(self._isMove && !dontFade && self.st.fadeinLoadedSlide  && ( bId === 0 || ( (visibleNearby || self._isAnimating || self._isDragging) && (bId === -1 || bId === 1) ) ) ) {
                ...
                setTimeout(function() {
                    content.css('opacity', 1);
                    //self.ev.trigger('scAfterContentSet', holder);
                }, 6);
            } else {
                self._slidesContainer.append(holder);
            }
        ...
        updateSliderSize: function(force) {
            ...
            if(force || newWidth != self.width || newHeight != self.height) {
                ...
                self.ev.trigger('scBeforeSizeSet');
        ...
        setSlidesOrientation: function(orient) {
            ...
            //      }
            //  }
            //  self.slider.removeClass(self._slidesHorizontal ? 'scHor' : 'scVer').addClass(newHor ? 'scHor' : 'scVer');
        ...
        appendSlide: function(htmltext, index) {
            ...
            self.slidesJQ.splice(index, 0, '<div style="'+ (self._isMove ? 'position: absolute;' : 'z-index: 0; display:none; opacity: 0; position: absolute;  left: 0; top: 0;' ) +'" class="scSlide"></div>');

            if(index < self.currSlideId) {
                self.currSlideId++;
            }
            self.ev.trigger('scOnAppendSlide', [parsedSlide, index]);
        ...
        removeSlide: function(index) {
            ...
            if(slideToRemove) {
                ...
                self.ev.trigger('scOnRemoveSlide', [index]);
                self._refreshSlides(index);
        ...
        _moveTo:function(type,  speed, inOutEasing, userAction, fromSwipe) {
            ...
            self.ev.trigger('scBeforeMove', [type, userAction]);
            if(type === 'next') {
        ...
        _animateTo:function(pos, dir,  loadAll, inOutEasing, customComplete) {
            ...
            if(!self._useCSS3Transitions) {
                if(self._isMove) {
                    ...
                    clearTimeouts();
                    if(oldBlock) {
                        oldBlock.data('scTimeout', setTimeout(function() {
                ...
                if(self._isMove) {
                    ...
                    animObj.opacity = 0;
                    animObj[self._vendorPref + self._TD] = '0ms';
                    animObj[self._vendorPref + self._TTF] = $.scCSS3Easing[self.st.easeInOut];
                    animBlock.css(animObj);
                    animBlock.data('scTimeout', setTimeout(function() {
                        //animBlock.css('opacity', 0);
        ...
        _resizeImage:function(slideObject, useClone) {
            ...
            if(imgScaleMode !== 'none') {
                ...
                if(isVideo) {
                    img.find('.scImg').css({width: '100%', height:'100%'});
                }
            }
            if (imgAlignCenter) {     
    ...
    }; /* SonyCarousel core prototype end */

    $.scProto = SonyCarousel.prototype;
