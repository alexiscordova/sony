...
var Backbone, BasePlugin, CSON, Collection, DocPad, DocumentModel, ElementsCollection, EventEmitterEnhanced, Events, FileModel, FilesCollection, MetaCollection, Model, PluginLoader, QueryCollection, ScriptsCollection, StylesCollection, View, airbrake, balUtil, caterpillar, growl, pathUtil, queryEngine, util, _, _ref,
...
DocPad = (function(_super) {
  ...
  */
  ...
  DocPad.prototype.load = function(instanceConfig, next) {
    ...
    preTasks = new balUtil.Group(function(err) {
      ...
      if ((_ref2 = (_base = _this.initialConfig).port) == null) {
        _base.port = (_ref3 = (_ref4 = (_ref5 = process.env.PORT) != null ? _ref5 : process.env.VCAP_APP_PORT) != null ? _ref4 : process.env.VMC_APP_PORT) != null ? _ref3 : 9778;
      }
      (_base1 = _this.initialConfig).env || (_base1.env = process.env.NODE_ENV || 'development');
  ...
  DocPad.prototype.compareVersion = function() {
    ...
    balUtil.packageCompare({
      ...
      newVersionCallback: function(details) {
        ...
        return docpad.log('notice', util.format(locale.upgradeDetails, details.local.version, details.remote.version, details.remote.homepage));
      }
    });
    return this;
  };
  ...
  DocPad.prototype.renderFiles = function(opts, next) {
    ...
    if (typeof opts.setProgressIndicator === "function") {
      opts.setProgressIndicator(function() {
        return ['renderFiles', tasks.completed, tasks.total];
      });
    }
    collection.forEach(function(file) {
      return tasks.push(function(complete) {
  ...
  DocPad.prototype.generateRender = function(opts, next) {
    var collection, docpad, setProgressIndicator, templateData, _ref1;
    _ref1 = balUtil.extractOptsAndCallback(opts, next), opts = _ref1[0], next = _ref1[1];
    docpad = this;
    templateData = opts.templateData || this.getTemplateData();
    collection = opts.collection || this.getDatabase();
    setProgressIndicator = opts.setProgressIndicator || null;
    balUtil.flow({
      object: docpad,
  ...
  DocPad.prototype.watch = function(opts, next) {
    var changeHandler, closeWatchers, database, docpad, ignorePatterns, locale, performGenerate, queueRegeneration, regenerateDelay, regenerateTimer, resetWatchers, watchr, watchrs, _ref1;
    ...
    if (this.config.ignorePatterns === true) {
    ...
    changeHandler = function(eventName, filePath, fileCurrentStat, filePreviousStat) {
      ...
      if (ignorePatterns && ignorePatterns.test(pathUtil.basename(filePath))) {
  ...
  DocPad.prototype.serverMiddlewareRouter = function(req, res, next) {
    var database, docpad, document, pageUrl, url;
    docpad = this;
    database = docpad.getDatabase();
    if (!database) {
      return next();
  ...
  DocPad.prototype.server = function(opts, next) {
    var complete, config, docpad, express, http, locale, serverExpress, serverHttp, startServer, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7, _ref8, _ref9;
    http = null;
    express = null;
    _ref1 = balUtil.extractOptsAndCallback(opts, next), opts = _ref1[0], next = _ref1[1];
    docpad = this;
    ...
    startServer = function() {
      ...
      try {
        ...
        return docpad.log('info', util.format(locale.serverStarted, serverLocation, serverDir));
      } catch (err) {
        return complete(err);
      } finally {
        return complete();
