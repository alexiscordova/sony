...
module.exports = function(grunt) {
  ...
  var jadeSpawn = function(files, options){

    var done = grunt.task.current.async(),
        spawnCount = options.processes,
    ...
    for ( var i = 0; i < spawnCount; i++ ) {
      ...
      for ( var j = startPoint; j < startPoint + filesPerProcess + endOffset; j++ ) {
        ...
        processTasks.push('jade-single:' + files[j].src[0] + ":" + grunt.task.current.target);
        filePosition++;
      }

      // If there are any current options, extend them onto the new process list. (--debug, --deploy, etc.)
  ...
  grunt.registerTask('jade-single', function(filePath, parentTask){

    if ( !filePath || !parentTask ) return;

    var files = grunt.config('jade.' + parentTask).files,
    ...
    if ( files[0] ) {
      ...
      for (var key in files) {
        ...
        if ( _.first(file) !== filePath ) {
          delete files[key];
        }
      }
    }

    delete options.processes;
